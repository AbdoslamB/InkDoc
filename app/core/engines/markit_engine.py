...
def is_markit_available() -> bool:
    ...
    # InkDoc bundles high-speed pure-Python broad-format extractors (.ipynb, .epub, .yaml, .csv, etc.)
    # ensuring Markit conversion capabilities are always available on all platforms without external dependencies.
    return True
...
def convert_with_markit(item: QueueItem, options: ConversionOptions) -> str:
    ...
    if sys.platform != "win32":
        ...
        if shutil.which("markit"):
            ...
            cmd = ["npx", "--yes", f"@shiftlabs/markit@{MARKIT_VERSION}", "-q", "--", source]
    ...
    if sys.platform == "win32" and shutil.which("wsl"):
        ...
        if wsl_path:
            try:
                res = subprocess.run(
                    ["wsl", "npx", "--yes", f"@shiftlabs/markit@{MARKIT_VERSION}", "-q", "--", wsl_path],
                    capture_output=True,
                    text=True,
                    timeout=60,
                    encoding="utf-8",
